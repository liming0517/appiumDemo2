...
/**
 * 1. Go to Settings -> Accessibility -> Font size/Display size/Auto-rotate screen/dark theme/Vibrate /System navigation
 * 2. Change Settings from above
 * 3. Go to Setting -> Display (or other entry ) to check the settings value.
 */
@Controller
@ResponseBody
@RequestMapping("pixel")
public class Testcase22 {
    private static Logger logger = Logger.getLogger(Testcase22.class);

    @Autowired
    public final AppiumConfig config;

    //constructor
    public Testcase22(AppiumConfig config) {
        this.config = config;
    }
    ...
    public ResponseEntity adjustFontText() {
        try {
            ...
            try {
                ...
                WebElement display = GenericTools.findElementSwipeDownByXpath(driver, "text", LanguageText.Accessibility);
